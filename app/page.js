...
import { Search, Github, AlertCircle, Loader2, Zap } from "lucide-react";
...
export default function Home() {
  ...
  const { analyze, status, isLoading, isComplete, result, reset } = useAnalysis();
  ...
  return (
    <main className={cn(
      "min-h-screen relative selection:bg-stone-100/10 font-sans antialiased text-stone-100 transition-colors",
      (!isComplete && !isLoading) ? "h-screen overflow-hidden" : "overflow-x-hidden"
    )}>
      ...
      <div className="relative h-full z-10">
        <AnimatePresence mode="wait">
          {!isComplete && !isLoading ? (
            /* Home State: Single Viewport */
            <motion.div
              key="landing"
              className="h-screen flex flex-col items-center justify-center container mx-auto px-6 relative"
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
            ...
            >
                    ...
                    {recent.map((item, i) => (
                      <div key={i} className="group relative">
                        <button
                          ...
                          className="px-5 py-2.5 rounded-xl bg-stone-800/60 backdrop-blur-sm border border-stone-700/50 hover:bg-stone-700/80 hover:border-stone-600 transition-all text-[11px] font-black text-stone-200 flex items-center gap-3 hover:shadow-lg hover:shadow-stone-900/20 hover:-translate-y-0.5"
                        >
                          <Github className="w-4 h-4 opacity-70 group-hover:opacity-100 transition-opacity" />
                          {String(item.repoFullName || item.url || '').toUpperCase()}
                        </button>
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                          ...
                          className="absolute -top-2 -right-2 w-5 h-5 rounded-full bg-red-500 text-white flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity shadow-lg hover:bg-red-600"
                        >
              ...
              {/* Minimal Branding */}
              <footer className="absolute bottom-8 left-0 right-0 flex flex-col items-center gap-3 select-none">
                <p className="text-xl font-heading font-black tracking-widest text-stone-500/50 uppercase">Gitara</p>
                <a
                  href="https://akhilesh.pages.dev"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="text-[9px] font-black text-stone-700 hover:text-stone-400 transition-colors uppercase tracking-[0.4em] hover:scale-110 transform duration-300 flex items-center gap-2"
                >
                  <span className="w-4 h-px bg-stone-800"></span>
                  Akhilesh
                  <span className="w-4 h-px bg-stone-800"></span>
                </a>
              </footer>
            ...
            <motion.div
              key="loading"
              className="h-screen flex flex-col items-center justify-center container mx-auto px-6"
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
            ...
            >
              ...
              <footer className="mt-40 pb-20 border-t border-stone-800/50 pt-20 flex flex-col items-center gap-4 select-none">
                <p className="text-5xl font-heading font-black tracking-tighter text-stone-600/20 uppercase">Gitara</p>
                <a
                  href="https://akhilesh.pages.dev"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="text-[10px] font-black text-stone-600 hover:text-stone-400 transition-colors uppercase tracking-[0.4em] hover:scale-110 transform duration-300 flex items-center gap-3"
                >
      ...
      <AnimatePresence>
        {status.error && (
          <motion.div
            className="fixed inset-0 z-[60] flex items-center justify-center p-6 bg-stone-950/40 backdrop-blur-md"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
          >
